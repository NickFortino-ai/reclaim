    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <form onSubmit={handleSubmit} className="card">
        ...
        <div className="mt-3">
          ...
          >
            <svg className={`w-4 h-4 transition-transform ${showMoods ? 'rotate-90' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
            </svg>
            How are you feeling?{mood && ` ${MOODS.find(m => m.value === mood)?.emoji || ''}`}
          </button>
          {showMoods && (
        ...
        <div className="mt-3">
          <button
            ...
            className="flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700"
          >
            <svg className={`w-4 h-4 transition-transform ${showTriggers ? 'rotate-90' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
            </svg>
            Any triggers today?{trigger && ` ${TRIGGERS.find(t => t.value === trigger)?.emoji || ''}`}
          </button>
          {showTriggers && (
            <div className="flex flex-wrap gap-2 mt-2">
              {TRIGGERS.map((t) => (
